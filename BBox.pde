static class Box implements UIE
{
  ...
  static Box BoxScaleLinesSeg( Cam cam, C lt, C ar, float scale, int lines, float seg )
  {
    C arsca = ar.sca( scale );
    return BoxLinesSeg( cam, lt.add( ar.sub( arsca ).sca( .5 ) ), arsca, lines, seg );
  }
